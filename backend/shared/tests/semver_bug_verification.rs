...
fn bug_1_pre_release_versions_parse_correctly() {
    ...
    assert!(SemVer::parse("1.0.0-beta.1").is_some(), "1.0.0-beta.1 should parse");
    assert!(SemVer::parse("2.0.0-rc.1").is_some(), "2.0.0-rc.1 should parse");
    assert!(SemVer::parse("1.0.0+build.123").is_some(), "1.0.0+build.123 should parse");
}
